...
export async function POST(request: NextRequest) {
	try {
		...
		if (!supabaseUrl || !supabaseServiceKey) {
			...
			return NextResponse.json(
				{ 
					error: "PDF storage is not configured. Please contact your administrator. Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY environment variables." 
				},
				{ status: 500 }
			);
		}
		...
		for (const file of files) {
			...
			const objectPath = `invoices/${(session.user as any).id || "user"}/${timestamp}-${sanitizedName}`;
