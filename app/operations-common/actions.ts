...
export async function getFolders(parentId?: string | null) {
  ...
  try {
    const folders = await prisma.operationsCommonFolder.findMany({
...
export async function getFiles(folderId?: string | null) {
  ...
  try {
    const files = await prisma.operationsCommonFile.findMany({
...
export async function createFolder(name: string, parentId?: string | null) {
  ...
  try {
    const folder = await prisma.operationsCommonFolder.create({
      data: {
        name: name.trim(),
...
export async function updateFolder(folderId: string, name: string) {
  ...
  try {
    const folder = await prisma.operationsCommonFolder.update({
      where: { id: folderId },
      data: {
        name: name.trim(),
      },
...
export async function deleteFolder(folderId: string) {
  ...
  try {
    ...
    const folder = await prisma.operationsCommonFolder.findUnique({
      where: { id: folderId },
      include: {
        _count: {
          select: {
    ...
    await prisma.operationsCommonFolder.delete({
      where: { id: folderId },
    });
...
export async function createFile(
  ...
  try {
    const file = await prisma.operationsCommonFile.create({
      data: {
        name: name.trim(),
        originalName,
        fileType,
...
export async function updateFile(fileId: string, name?: string, folderId?: string | null) {
  ...
  try {
    ...
    const file = await prisma.operationsCommonFile.update({
      where: { id: fileId },
      data: updateData,
      include: {
        creator: {
...
export async function deleteFile(fileId: string) {
  ...
  try {
    const file = await prisma.operationsCommonFile.findUnique({
      where: { id: fileId },
    });
    ...
    await prisma.operationsCommonFile.delete({
      where: { id: fileId },
    });
